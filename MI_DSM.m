...
% It is implemented on BCI competition 4 dataset 2b. 
% It is a generalized code which can be implemented on any datasets. 
% The dimesion of the subspaces is tried on all values, which can be 
...
for d=1:2*Ch-1
    ...
    % Linear Subspace model of optimum second segment
...
for epoch=1:length(st)
    ...
    clear UDS; clear EDS;
    [UDS,EDS,~]=svd(P+Q);
    ID=find(diag(EDS)<1); % Bases of difference subspace (DS)
...
% Optimum model training and Testing the model on test data
